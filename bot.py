...
if __name__ == "__main__":
    ...
    loop.run_until_complete(start_services())        if seconds == 0 and remainder == 0:
            break
        time_list.append(int(result))
        seconds = int(remainder)
    for x in range(len(time_list)):
...
async def shortener_callback_handler(c: StreamBot, cb):
    ...
    # Thumbnail downloading disabled to avoid Telegram FloodWait
    # if getattr(file, 'thumbs', None):
    #     try:
    #         thumb_path = await c.download_media(file.thumbs[0].file_id)
    #         await cb.message.reply_photo(
    ...
    # Send directly without thumbnail
    await cb.message.reply_text(
        caption_text, 
        reply_markup=InlineKeyboardMarkup(buttons), 
        disable_web_page_preview=True,
